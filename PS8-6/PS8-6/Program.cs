...
namespace PS8_6
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for(int i = 0; i < numOfPeople; ++i)
            {
                ...
                Int32.TryParse(currLineTokens[1], out int currTime);
                Int32.TryParse(currLineTokens[0], out int currMoney);

                if (dict.ContainsKey(currTime))
                {
                    dict[currTime].Add(currMoney);
                }
                else
                {
                    MaxHeap heap = new MaxHeap(numOfPeople);
                    heap.Add(currMoney);
                    dict.Add(currTime, heap);
                }
            }
    ...
    /// <summary>
    /// I grabbed this max heap implementation from the link below
